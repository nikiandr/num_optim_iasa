...
def PlotContour(x_lim, y_lim, target_func, points=None, fname=None):
    ...
    plt.contour(domain_x, domain_y, func, levels=10, cmap='viridis')
    if points is not None:
        plt.plot(points[:, 0], points[:, 1], color='tab:red', linewidth=1)
        plt.scatter(points[-1, 0], points[-1, 1], color='tab:red', marker='x',
                    s=20)
        plt.scatter(points[0, 0], points[0, 1], color='tab:red', marker='o',
                    s=20)
